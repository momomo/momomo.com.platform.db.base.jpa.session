 ...
 *
 * Anytime get currentSession() is called, the last one to have registered is the one to use by anyone 'requiring' a session. 
 * 
 * This also ensures that calls to openSession() which for our Transactional API means, start a new transaction, will also be registered.
 * 
 * Currently, with Hibernate there is no way to reuse a session if you required a new one, which for newTransaction(...) would mean that a subsequent or nested call to requireTransaction would not recognize it, and hence return a new session itself.
 * 
 * This is a flaw in Hibernate's **** API. 
 * It is a shame that major major parts of Hiberante API and classes are a) so complex (unnecessarily) , b) declare private fields, methods and classes everywhere making extending them very complicated.  
 *
 * If you require advanced transaction capabilities, ie, nested transactions such as starting a new transaction while you have ongoing one, we suggest you use this one. 
 * If not, you should be using the almost identical one {@link $SessionConfigContextSingleCrazyInsane} but which prevents Hibernate from wrapping the session instance normally done at times by Hibernate in {@link org.hibernate.context.internal.ThreadLocalSessionContext} to a wrapped and limited Session as {@link org.hibernate.context.internal.ThreadLocalSessionContext.TransactionProtectionWrapper}. 
 * This is highly problematic, as you can not make certain method calls on the session which are perfectly valid to do, such as doWork(connection -> {}). 
 * 
 * We also provide our own {@link momomo.com.db.sessionfactory.$SessionFactory} implementation that also synchronizes the main openSession() call as well which the default {@link org.hibernate.internal.SessionFactoryImpl} does not.  
 *
 * All our classes that implement our base class {@link $SessionConfigContextBase} such as
 * 
 *   - {@link momomo.com.db.$SessionConfigContextListRecommended}
 *   - {@link momomo.com.db.$SessionConfigContextSingleCrazyInsane}  # Multiple sessions are not allowed. Based on Hibernates limited implementation which rollbacks and terminates any previous existing session on threadlocal while creating / opening a new one. Eats exception should there be one while rolling back and only logs it Crazy, insane? They even roll you back! And if there is no error you won't know they rolled you! If there is an error, it is basically silent! No bubbling earning it the name Insane.  
 *   - {@link momomo.com.db.$SessionConfigContextSingleCrazySane}    # Multiple sessions are not allowed. Based on Hibernates limited implementation which rollbacks and terminates any previous existing session on threadlocal while creating / opening a new one. Throws exception should there be one while rolling back. Crazy, sane? We still think rolling back is crazy, so we retain the crazy name, but at least is not is not silent, as we throw the rollback exception so it bubbles up should there be one, earning it the name Sane.  
 *   - {@link momomo.com.db.$SessionConfigContextSingleCrazyLaxed}   # Multiple sessions are allowed    . Based on Hibernates limited implementation which does not terminate or do anything with previous session, allows it to coexist. No rollback occurs. The developer might still terminate, commit and manually handle any previous transaction created. Still Crazy, since when you close the new session, the previous one can not be accessed, despite the thread actually having one. Crazy, but Laxed.    
 *
 ...
 * but you still have to use an implementation based on {@link momomo.com.db.$SessionConfigContextBase}.
 * 
 ...
 * 
 * which is already default in our {@link momomo.com.db.$SessionConfig#properties()}
 * 
 * You could also use one of the others
 * 
 *   properties.put(Environment.CURRENT_SESSION_CONTEXT_CLASS, {@link $SessionConfigContextSingleCrazyInsane}.class.getName());
 *   properties.put(Environment.CURRENT_SESSION_CONTEXT_CLASS, {@link $SessionConfigContextSingleCrazySane}.class.getName());
 *   properties.put(Environment.CURRENT_SESSION_CONTEXT_CLASS, {@link $SessionConfigContextSingleCrazyLaxed}.class.getName());
 *   
 ...
 *   properties.put(Environment.CURRENT_SESSION_CONTEXT_CLASS, {@link $SessionConfigContextSingleCrazyInsane}.class.getName());
 *  
 *  Only difference between {@link $SessionConfigThreadLocalSessionContextUnwrappedUntrackedNotRecommended} and {@link $SessionConfigContextSingleCrazyInsane} is that the latter will also track openSession calls. 
 *  
 * @author Joseph S.
 */
